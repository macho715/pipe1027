...
def calculate_pareto_analysis(
    aisle_data: pd.DataFrame, threshold: float = 80.0
) -> Dict:
    """
    Aisle별 Pareto 분석 수행
    ...
    recommendations = generate_optimization_recommendations(
        pareto_aisles, df_sorted, df, threshold
    )
...
def calculate_aisle_occupancy(df_wh: pd.DataFrame) -> pd.DataFrame:
    ...
    # Aggregate by aisle
    aisle_occupancy = df_wh.groupby("aisle_code").agg(
        {"area_sqm": "sum", "aisle_code": "count"}
