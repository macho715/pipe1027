...
========================
Column names and header matching follow @core/ centralized system.
Stage 4 reads data from Stage 3 output, which already uses core-standardized headers.

If you need to match/find columns in the input DataFrame:
- Use SemanticMatcher from @core/ for flexible column matching
- Use HeaderNormalizer for handling header name variations
- Reference HVDC_HEADER_REGISTRY for known column definitions

DO NOT hardcode column names. Use semantic matching or normalized lookup.
Column name variations (whitespace, case, aliases) are handled by core.
...
class DetectorConfig:
    ...
    def __post_init__(self):
        ...
        # Core에서 창고/현장 목록을 가져와 대문자+언더스코어 형식으로 변환
        if self.warehouse_columns is None:
            self.warehouse_columns = [
                wh.replace(" ", "_").upper() for wh in get_warehouse_columns()
            ]
        if self.site_columns is None:
            # 현장은 이미 대문자 형식이므로 그대로 사용
...
class DataQualityValidator:
    ...
    def validate(self, df: pd.DataFrame) -> List[str]:
        ...
        # 날짜 변환 가능성(창고/현장) - Core에서 동적으로 가져오기
        warehouse_columns = [
            wh.replace(" ", "_").upper() for wh in get_warehouse_columns()
        ]
...
class StatDetector:
    ...
    def per_location_outliers(self, dwell_list: List[Tuple[str, str, int]]) -> List[AnomalyRecord]:
        """
        위치별 IQR(+MAD 보정) 임계로 과도 체류 판정
        사용자 요구사항에 따라 상태 체류 탐지 비활성화
        """
        # 상태 체류 탐지 비활성화 - 사용자 요구사항
...
class RuleDetector:
    ...
    def time_reversal(self, row: pd.Series) -> Optional[AnomalyRecord]:
        """
        시간 역전 탐지: ETD/ETA > 창고/현장 입고일

        Args:
            row: DataFrame row containing date columns

        Returns:
            AnomalyRecord: Anomaly info if detected, None otherwise
        """
        # ETD, ETA 가져오기
        etd = pd.to_datetime(row.get("ETD"), errors="coerce") if "ETD" in row.index else None
        eta = pd.to_datetime(row.get("ETA"), errors="coerce") if "ETA" in row.index else None

        # 창고 입고일 가져오기 (여러 가능한 컬럼명 확인)
        warehouse_date = None
        for col in ["Status_WAREHOUSE_Date", "Warehouse_Inbound_Date", "Status_WAREHOUSE"]:
            if col in row.index and pd.notna(row[col]):
                warehouse_date = pd.to_datetime(row[col], errors="coerce")
                if pd.notna(warehouse_date):
                    break

        # 현장 입고일 가져오기 (여러 가능한 컬럼명 확인)
        site_date = None
        for col in ["Status_SITE_Date", "Site_Inbound_Date", "Status_SITE"]:
            if col in row.index and pd.notna(row[col]):
                site_date = pd.to_datetime(row[col], errors="coerce")
                if pd.notna(site_date):
                    break

        # ETD > 창고 입고일 체크
        if etd and warehouse_date and etd > warehouse_date:
            desc = f"ETD({etd.date()})가 창고입고일({warehouse_date.date()})보다 늦음"
        ...
        # ETA > 창고 입고일 체크
        if eta and warehouse_date and eta > warehouse_date:
            desc = f"ETA({eta.date()})가 창고입고일({warehouse_date.date()})보다 늦음"
            return AnomalyRecord(
                case_id=str(row.get("CASE_NO", "NA")),
                anomaly_type=AnomalyType.TIME_REVERSAL,
                severity=AnomalySeverity.CRITICAL,
        ...
        # ETD > 현장 입고일 체크
        if etd and site_date and etd > site_date:
            desc = f"ETD({etd.date()})가 현장입고일({site_date.date()})보다 늦음"
            return AnomalyRecord(
                case_id=str(row.get("CASE_NO", "NA")),
                anomaly_type=AnomalyType.TIME_REVERSAL,
                severity=AnomalySeverity.CRITICAL,
        ...
        # ETA > 현장 입고일 체크
        if eta and site_date and eta > site_date:
            desc = f"ETA({eta.date()})가 현장입고일({site_date.date()})보다 늦음"
            return AnomalyRecord(
                case_id=str(row.get("CASE_NO", "NA")),
                anomaly_type=AnomalyType.TIME_REVERSAL,
                severity=AnomalySeverity.CRITICAL,
...
class HybridAnomalyDetector:
    ...
    def run(
        ...
        self.comb.ingest_rule([a for a in anomalies if a.anomaly_type == AnomalyType.TIME_REVERSAL])
        self.comb.ingest_stat(stat_recs)

        # ML
        use_cols = [
            c for c in ["TOUCH_COUNT", "TOTAL_DAYS", "AMOUNT", "QTY", "PKG"] if c in feat.columns
        ]
            ...
            for case_id, yi, ri in zip(X.index, y, risk):
                if yi == 1:
                    ...
                    sev = (
                        ...
                        if fused >= 0.97
                        else (AnomalySeverity.HIGH if fused >= 0.90 else AnomalySeverity.MEDIUM)
                    )
                    anomalies.append(
                        AnomalyRecord(
                            case_id=str(case_id),
