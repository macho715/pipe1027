...
class HeaderDetectionResult:
    """헤더 탐지 결과 및 진단 정보를 보관합니다. | Store header detection outcome and diagnostics."""
    ...
    def is_confident(self) -> bool:
        """탐지 결과가 임계치를 충족하는지 여부. | Whether detection meets the threshold."""

        return self.confidence >= self.threshold

    def to_tuple(self) -> Tuple[int, float]:
        """(row_index, confidence) 튜플 반환. | Return classic (row_index, confidence) tuple."""
...
class HeaderDetector:
    ...
    def detect_with_diagnostics(
        ...
        """엑셀 파일을 분석하여 진단 정보와 함께 헤더 위치를 반환합니다. | Detect header with diagnostics from file."""
        ...
        if expected_columns:
            row_index, confidence = self.detect_with_column_names(df, expected_columns)
            method = "heuristic+semantic"
        else:
            row_index, confidence = self.detect_from_dataframe(df)
            method = "heuristic"
        ...
        if confidence < self.min_confidence:
            warnings.append(
                (
                    "LOW_CONFIDENCE"
                    f": score {confidence:.2f} below threshold {self.min_confidence:.2f}"
                )
            )
        ...
        return HeaderDetectionResult(
            ...
            method=method,
            warnings=warnings,
            diagnostics={"rows_analyzed": float(len(df))},
        )
    ...
    def _calculate_keyword_score(self, row: pd.Series) -> float:
        ...
        keyword_matches = sum(1 for keyword in self.header_keywords if keyword in row_text)
    ...
    def _validate_data_rows(self, df: pd.DataFrame, header_idx: int) -> float:
        ...
        for i in range(1, data_rows_to_check + 1):
            ...
            text_ratio = sum(1 for val in data_row.dropna() if isinstance(val, str)) / max(
                1, data_row.notna().sum()
            )
    ...
    def detect_with_column_names(
        ...
        normalized_detected = [
            normalize_header(str(col)) for col in detected_headers if pd.notna(col)
        ]
...
def detect_header_row(
    ...
    """간단한 헤더 탐지 인터페이스. | Lightweight header detection helper."""
...
def detect_header_row_with_diagnostics(
    ...
    """진단 정보와 함께 헤더를 탐지합니다. | Detect headers with diagnostics."""

    detector = HeaderDetector()
    return detector.detect_with_diagnostics(
        file_path=file_path,
        sheet_name=sheet_name,
        expected_columns=expected_columns,
    )
