...
class MatchResult:
    ...
    def __str__(self) -> str:
        ...
        if self.matched:
            return (
                f"✓ {self.semantic_key} → '{self.column_name}' "
...
class MatchReport:
    ...
    def print_summary(self):
        ...
        print(
            f"  Success rate:            {self.successful_matches/max(1,self.total_semantic_keys):.1%}"
        )
        ...
        if failed:
            ...
            for result in failed:
                ...
                if result.alternatives:
                    print(
                        f"    Alternatives: {', '.join(col for col, _ in result.alternatives[:3])}"
                    )
...
class SemanticMatcher:
    ...
    def match_dataframe(
        ...
        for key in semantic_keys:
            try:
                result = self._match_single_key(
                    key, df.columns.tolist(), normalized_columns, matched_columns
                )
        ...
        if report.successful_matches > 0:
            report.average_confidence = np.mean([r.confidence for r in results if r.matched])

        return report

    def find_column(
        self, df: pd.DataFrame, semantic_key: str, required: bool = False
    ) -> Optional[str]:
        """
    ...
    def _normalize_dataframe_columns(self, df: pd.DataFrame) -> Dict[str, str]:
        """
    ...
    def _match_single_key(
        ...
        if self.allow_partial:
            partial_matches = self._find_partial_matches(
                normalized_aliases, normalized_columns, already_matched
            )
            ...
            if partial_matches:
                ...
                if score >= self.min_confidence:
                    ...
                    result.alternatives = [
                        (normalized_columns[nc], s) for nc, s in partial_matches[1:3]
                    ]
                    return result
                else:
                    # Score too low, but record alternatives
                    result.alternatives = [
                        (normalized_columns[nc], s) for nc, s in partial_matches[:3]
                    ]
    ...
    def is_semantically_similar(self, header1: str, header2: str, threshold: float = 0.8) -> bool:
        """
        Check if two headers are semantically similar.

        Args:
            header1: First header name
            header2: Second header name
            threshold: Similarity threshold (0-1)

        Returns:
            True if headers are semantically similar
        """
        # Normalize both headers
        norm1 = self.normalizer.normalize(header1)
        norm2 = self.normalizer.normalize(header2)
        ...
        score = self._calculate_similarity_score(norm1, norm2)
...
def find_header_by_meaning(
    df: pd.DataFrame, semantic_key: str, required: bool = False, registry=None
) -> Optional[str]:
    """
...
if __name__ == "__main__":
    ...
    for i, test in enumerate(test_cases, 1):
        ...
        for result in report.results:
            if result.matched:
                print(
                    f"  ✓ {result.semantic_key:20s} → '{result.column_name}' ({result.confidence:.0%})"
                )
            else:
