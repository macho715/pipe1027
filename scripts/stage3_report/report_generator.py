...
# ============================================================================
# HEADER MANAGEMENT POLICY: CENTRALIZED IN @core/
# ============================================================================
# All header/column definitions, warehouse lists, and site lists are managed
# centrally in @core/header_registry.py to ensure consistency across stages.
#
# This file uses:
# - HVDC_HEADER_REGISTRY - Central registry for all header definitions
# - HeaderNormalizer - Handles whitespace, case, alias matching
# - _warehouse_labels() from flow_ledger_v2 - Gets warehouse display names
# - normalize_header_names_for_stage3() - Applies core normalization rules
#
# DO NOT hardcode warehouse names or column lists in this file.
# ALWAYS use registry methods or imported functions to get current definitions.
#
...
class CorrectedWarehouseIOCalculator:
    ...
    def __init__(self, use_vectorized=False, use_parallel=False):
        ...
        self.unified_end_month = None  # Will be set from raw data for consistent reporting
        ...
        #  수정: 창고와 현장을 Core에서 동적으로 가져오기
        # 헤더 관리는 @core/header_registry.py에서 중앙집중식으로 관리됩니다
        self.warehouse_columns = get_warehouse_columns()
        self.site_columns = get_site_columns()

        #  수정: 위치 우선순위 (타이브레이커용) - Core 기반 동적 생성
        self.location_priority = {}
        priority = 1
        for wh in self.warehouse_columns:
            self.location_priority[wh] = priority
            priority += 1
        for site in self.site_columns:
            self.location_priority[site] = priority
            priority += 1

        # 창고 우선순위 - Core 기반 동적 생성
        # Core의 순서를 따르되, DHL Warehouse 별칭 처리
        self.warehouse_priority = [
            wh if wh != "DHL WH" else "DHL Warehouse" 
            for wh in self.warehouse_columns
        ]
        # DSV MZD 추가 (레거시 호환성)
        if "DSV MZD" not in self.warehouse_priority:
            # DSV MZP 뒤에 DSV MZD 삽입
            try:
                idx = self.warehouse_priority.index("DSV MZP")
                self.warehouse_priority.insert(idx + 1, "DSV MZD")
            except ValueError:
                self.warehouse_priority.append("DSV MZD")

        #  FIX 1: SQM 기반 창고 관리 설정 - Core 기반 동적 생성
        # 기본값 설정 (업무 로직)
        default_sqm_config = {
            "DSV Al Markaz": 12000,
            "DSV Indoor": 8500,
        ...
        # Core 창고 목록 기반으로 동적 생성
        self.warehouse_base_sqm = {}
        for wh in self.warehouse_columns:
            # DHL WH와 DHL Warehouse 모두 처리
            self.warehouse_base_sqm[wh] = default_sqm_config.get(
                wh, default_sqm_config.get("DHL Warehouse" if wh == "DHL WH" else wh, 1000)
            )
        # 레거시 호환성: DSV MZD 추가
        if "DSV MZD" not in self.warehouse_base_sqm:
            self.warehouse_base_sqm["DSV MZD"] = 1000
    ...
    def load_real_hvdc_data(self):
        ...
        try:
            ...
            if self.hitachi_file.exists():
                ...
                duplicate_cols = hitachi_data.columns[hitachi_data.columns.duplicated()].tolist()
                if duplicate_cols:
                    print(f"\n[WARN] 중복 컬럼 발견: {duplicate_cols}")
                    hitachi_data = hitachi_data.loc[:, ~hitachi_data.columns.duplicated()]
                    print(f"[OK] 중복 제거 후 컬럼 수: {len(hitachi_data.columns)}")
                ...
                # Stage 2 출력은 HITACHI와 SIEMENS를 모두 포함하고 있으므로
                # 기존 Source_Vendor 값을 덮어쓰지 않음
                ...
                for warehouse in self.warehouse_columns:
                    if warehouse in hitachi_data.columns:
                        col_data = hitachi_data[warehouse]
                        # Ensure we have a Series, not a DataFrame
                        if isinstance(col_data, pd.DataFrame):
                            col_data = col_data.iloc[:, 0]  # Take first column if DataFrame
            ...
            if self.simense_file.exists():
                ...
                # ✅ Source_Vendor 사용 (Stage 1-2와 일관성)
                simense_data["Source_Vendor"] = normalize_vendor_name("SIEMENS")
            ...
            if combined_dfs:
                ...
                for warehouse in self.warehouse_columns:
                    if warehouse not in self.combined_data.columns:
                        ...
                        col_data = self.combined_data[warehouse]
                        # Ensure we have a Series, not a DataFrame
                        if isinstance(col_data, pd.DataFrame):
                            col_data = col_data.iloc[:, 0]  # Take first column if DataFrame
                            # Update the actual column
                            self.combined_data[warehouse] = col_data
                        non_null_count = col_data.notna().sum()
                        print(f"    {warehouse}: {non_null_count}건 데이터")
                ...
                # Calculate unified end month from raw data (for consistent reporting)
                date_columns = (
                    list(self.warehouse_columns) +
                    list(self.site_columns) +
                    ["ETD/ATD", "ETA/ATA", "Status_Location_Date"]
                )
                from core.data_parser import calculate_max_date_from_dataframe
                self.unified_end_month = calculate_max_date_from_dataframe(
                    self.combined_data,
                    date_columns
                )
                logger.info(f" 통일된 보고서 기간: 2023-02 ~ {self.unified_end_month} (RAW 데이터 기준)")
            else:
    ...
    def process_real_data(self):
        ...
        # Remove duplicate columns before date conversion
        duplicate_cols = self.combined_data.columns[self.combined_data.columns.duplicated()].tolist()
        if duplicate_cols:
            print(f"\n[WARN] 날짜 변환 전 중복 컬럼 발견: {duplicate_cols}")
            self.combined_data = self.combined_data.loc[:, ~self.combined_data.columns.duplicated()]
            print(f"[OK] 중복 제거 후 컬럼 수: {len(self.combined_data.columns)}")
        ...
        for col in date_columns:
            if col in self.combined_data.columns:
                ...
                # Ensure we have a Series, not a DataFrame
                if isinstance(col_data, pd.DataFrame):
                    print(f"[WARN] {col} 컬럼이 DataFrame입니다. 첫 번째 컬럼 사용")
                    col_data = col_data.iloc[:, 0]
                self.combined_data[col] = pd.to_datetime(col_data, errors="coerce")
    ...
    def _calculate_warehouse_inbound_vectorized(self, df: pd.DataFrame) -> Dict:
        ...
        오리지널 로직 복원:
        - 모든 창고 입고를 포함 (필터링 제거)
        - Inbound_Type을 명시적으로 설정 (필터링 아님)
        - 창고간 이동 목적지만 제외
        """
        logger.info(" Hybrid 창고 입고 계산 시작 (루프 기반 입고 + 벡터화 창고간 이동)")
        ...
        for idx, row in df.iterrows():
            # 이 행의 창고간 이동 정보 가져오기
            row_transfers = (
                transfers_flat[transfers_flat["Row_ID"] == idx]
                if not transfers_flat.empty
                else pd.DataFrame()
            )
            transfer_destinations = (
                set(row_transfers["to_warehouse"].tolist()) if not row_transfers.empty else set()
            )
            ...
            for warehouse in self.warehouse_columns:
                if warehouse in row.index and pd.notna(row[warehouse]):
                    ...
                    try:
                        arrival_date = pd.to_datetime(row[warehouse])
                        pkg_quantity = int(row.get("Pkg", 1)) if pd.notna(row.get("Pkg")) else 1
                        ...
                        inbound_items.append(
                            {
                                ...
                                "Year_Month": arrival_date.strftime("%Y-%m"),
                                "Pkg_Quantity": pkg_quantity,
                                "Inbound_Type": "external_arrival",  # 명시적 설정 (필터링 아님)
                            }
                        )
        ...
        logger.info(
            f" Hybrid 창고 입고 계산 완료: {total_inbound}건 (창고간 이동 {len(warehouse_transfers)}건 별도)"
        )
    ...
    def _calculate_warehouse_outbound_vectorized(self, df: pd.DataFrame) -> Dict:
        ...
        # 2. 창고→현장 출고 처리 (완전 루프 기반 - 오리지널 방식)
        for idx, row in df.iterrows():
            # 이 행의 창고간 이동으로 이미 출고된 창고 추적 (오리지널 로직)
            row_transfers = []
            if not transfers_flat.empty:
            ...
            for warehouse in self.warehouse_columns:
                ...
                if warehouse in row.index and pd.notna(row[warehouse]):
                    try:
                        ...
                        logger.warning(f"출고 계산 오류 (Row {idx}, Warehouse {warehouse}): {e}")
                        continue
    ...
    def create_monthly_inbound_pivot(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # 월별 기간 생성 (RAW 데이터 최종 날짜까지 동적 계산)
        end_month = self.unified_end_month or datetime.now().strftime("%Y-%m")
    ...
    def calculate_cumulative_sqm_inventory(self, sqm_inbound: Dict, sqm_outbound: Dict) -> Dict:
        ...
        # 월별 순서로 처리 (unified_end_month까지 포함)
        all_months_from_data = set(list(sqm_inbound.keys()) + list(sqm_outbound.keys()))
        if all_months_from_data and self.unified_end_month:
            # Generate all months from start to unified_end_month
            start_month = min(all_months_from_data)
            all_months_range = pd.date_range(start_month, self.unified_end_month, freq="MS")
            all_months = sorted([m.strftime("%Y-%m") for m in all_months_range])
        else:
            all_months = sorted(all_months_from_data)
...
# === 창고 정규화 헬퍼 함수 (HeaderRegistry 기반) ===

def _warehouse_keys() -> Tuple[str, ...]:
    """
    Get warehouse semantic keys from core registry (Single Source of Truth).
    
    All warehouse definitions are centrally managed in @core/header_registry.py
    to ensure consistency across all pipeline stages.
    """
    reg = HVDC_HEADER_REGISTRY
    return tuple(reg.get_warehouse_columns(use_primary_alias=False))
...
def _alias_map_normalized() -> Dict[str, str]:
    ...
    for d in _warehouse_defs():
        ...
        for alias in d.aliases:
            k = normalizer.normalize(alias)  # ex) "dsvindoor", "dhlwarehouse"
            amap[k] = canonical
        # 정식 라벨 자체도 키로 허용
...
def _canon_warehouse(value: object) -> Optional[str]:
    """
...
def _build_latest_snapshot_from_master(stats: Dict) -> Dict[str, int]:
    """
    ...
    for key in ("master", "case_master", "master_rows", "final_table"):
        ...
        # stats["processed_data"]에서 계산 시도
        processed = stats.get("processed_data")
    ...
    loc_col = next(
        (
            ...
            in ("final_location", "status_location_final", "location_final")
        ),
        None,
    )
    qty_col = next(
        (
            c
            for c in df.columns
            if c.lower().replace(" ", "_") in ("pkg", "pkg_quantity", "quantity", "qty")
        ),
        None,
    )
    ...
    WAREHOUSE_LABELS = {
        d.description
        for d in [
            HVDC_HEADER_REGISTRY.get_definition(k)
            for k in (
                "dhl_wh",
                "dsv_indoor",
                "dsv_al_markaz",
...
def _anchor_cumulative_to_snapshot(
    df_monthly: pd.DataFrame, snapshot: Dict[str, int]
) -> pd.DataFrame:
    """
    ...
    """
    out = df_monthly.copy()
    # 대상 창고 추출: "누적_XXX" 패턴에서 창고명 뽑기
    cum_cols = [c for c in out.columns if c.startswith("누적_")]
    warehouses = [c.replace("누적_", "") for c in cum_cols]
    ...
    for w in warehouses:
        ...
        # Total 행 제외 (마지막에서 두 번째까지가 실제 월 데이터)
        last_idx = (
            len(out) - 2
            if len(out) > 0 and out.iloc[-1][out.columns[0]] == "Total"
            else len(out) - 1
        )

        # 스냅샷 앵커링
        last_flow = int(out[cum_col].iloc[last_idx])
        last_snap = int(snapshot.get(w, last_flow))
        delta = last_snap - last_flow
        if delta != 0:
            # 누적 컬럼 전체에 delta 추가 (Total 행 제외)
            mask = out.iloc[:, 0] != "Total"
            out.loc[mask, cum_col] = out.loc[mask, cum_col] + delta
...
class HVDCExcelReporterFinal:
    ...
    def __init__(self):
        ...
        self.unified_end_month = None  # Will be set from calculator after data loading
    ...
    def calculate_warehouse_statistics(self) -> Dict:
        ...
        # Copy unified_end_month from calculator for consistent reporting
        self.unified_end_month = self.calculator.unified_end_month
    ...
    def _get_warehouse_capacity(self, warehouse: str) -> int:
        """창고별 최대 용량 반환 (케이스 단위)
        ...
        return WAREHOUSE_CAPACITY.get(warehouse, 1000)
    ...
    def create_warehouse_monthly_sheet_enhanced(self, stats: Dict) -> pd.DataFrame:
        """
        Flow Ledger 기반 창고_월별_입출고 시트 생성
        """
        logger.info("창고_월별_입출고 시트 생성(Flow Ledger 기반)")

        # Flow ledger-based monthly IN/OUT/cumulative generation
        master_df = stats.get("processed_data")
        if master_df is None or (isinstance(master_df, pd.DataFrame) and master_df.empty):
            master_df = stats.get("master")
        if master_df is None or (isinstance(master_df, pd.DataFrame) and master_df.empty):
            master_df = stats.get("case_master")
        if master_df is None or (isinstance(master_df, pd.DataFrame) and master_df.empty):
            master_df = getattr(self, "combined_data", None)
        if master_df is None or (isinstance(master_df, pd.DataFrame) and master_df.empty):
            master_df = pd.DataFrame()
        logger.info(f"[DEBUG] master_df shape: {master_df.shape if not master_df.empty else 'EMPTY'}")
        logger.info(f"[DEBUG] stats keys: {list(stats.keys())}")
        
        ledger_df, edges_df = build_flow_ledger(master_df)
        warehouse_monthly = monthly_inout_table(ledger_df, _warehouse_labels(), end_month=self.unified_end_month)

        # Add Total row (average for utilization, sum for others)
        total_row = ["Total"]
        for col in warehouse_monthly.columns[1:]:
            if "이용률" in col:
                total_row.append(round(warehouse_monthly[col].mean(), 2))
            else:
                total_row.append(int(warehouse_monthly[col].sum()))
        warehouse_monthly.loc[len(warehouse_monthly)] = total_row

        # Sanity check
        bad = sanity_report(warehouse_monthly)
        if bad:
            logger.warning(f"[FlowLedger mismatch] {bad[:5]}")

        logger.info(f"✓ 창고_월별_입출고 시트 완료 (누적+이용률): {warehouse_monthly.shape}")
        return warehouse_monthly

    def create_warehouse_monthly_sheet_enhanced_OLD(self, stats: Dict) -> pd.DataFrame:
        """
        OLD VERSION - Kept for reference (v4.0.34 fallback logic)
        """
        logger.info("창고_월별_입출고 시트 생성(레지스트리/노멀라이저 기반 - Fallback)")
        ...
        def _mk_inbound_df() -> pd.DataFrame:
            ...
            if not in_items.empty:
                t = in_items.copy()
                t["Year_Month"] = t.get("Year_Month", pd.Series(dtype=str)).astype(str)
                t["Warehouse"] = t.get("Warehouse", pd.Series(dtype=object))
                t["Qty"] = t.get("Pkg_Quantity", pd.Series(dtype=float, index=t.index)).fillna(1)
                if "Inbound_Type" in t.columns:
                    t = t[t["Inbound_Type"] == "external_arrival"]
            ...
            if not wh_tx.empty:
                ...
                t["Warehouse"] = t.get(to_wh_col, pd.Series(dtype=object)).map(_canon_warehouse)
                t["Qty"] = t.get(qty_col, pd.Series(dtype=float, index=t.index)).fillna(1)
                t = t[["Year_Month", "Warehouse", "Qty"]].dropna(subset=["Warehouse"])
                frames.append(t)
        ...
        def _mk_outbound_df() -> pd.DataFrame:
            ...
            if not wh_tx.empty:
                ...
                t["Year_Month"] = t.get("Year_Month", pd.Series(dtype=str)).astype(str)
                from_wh_col = (
                    "from_warehouse" if "from_warehouse" in t.columns else "From_Warehouse"
                )
                qty_col = "pkg_quantity" if "pkg_quantity" in t.columns else "Pkg_Quantity"
                t["Warehouse"] = t.get(from_wh_col, pd.Series(dtype=object)).map(_canon_warehouse)
                t["Qty"] = t.get(qty_col, pd.Series(dtype=float, index=t.index)).fillna(1)
                t = t[["Year_Month", "Warehouse", "Qty"]].dropna(subset=["Warehouse"])
                frames.append(t)
            ...
            if not out_items.empty:
                t = out_items.copy()
                t["Year_Month"] = t.get("Year_Month", pd.Series(dtype=str)).astype(str)
                t["Warehouse"] = t.get("From_Location", pd.Series(dtype=object)).map(
                    _canon_warehouse
                )
                qty1 = t.get("Pkg_Quantity", pd.Series(dtype=float, index=t.index))
                qty2 = t.get("pkg_quantity", pd.Series(dtype=float, index=t.index))
                t["Qty"] = qty1.fillna(qty2).fillna(1)
                t = t[["Year_Month", "Warehouse", "Qty"]].dropna(subset=["Warehouse"])
                frames.append(t)
        ...
        if not inbound.empty:
            ...
            in_pvt = pd.DataFrame(index=month_keys, columns=warehouses).fillna(0)

        if not outbound.empty:
            out_pvt = (
                outbound.groupby(["Year_Month", "Warehouse"])["Qty"].sum().unstack(fill_value=0)
            )
        else:
            out_pvt = pd.DataFrame(index=month_keys, columns=warehouses).fillna(0)

        # 4) 인덱스/컬럼 정렬(월 전체, 창고 순서 고정)
        for m in month_keys:
            if m not in in_pvt.index:
                in_pvt.loc[m] = 0
            if m not in out_pvt.index:
                out_pvt.loc[m] = 0
        ...
        # 5) 누적 재고 및 이용률 계산
        cumulative = pd.DataFrame(index=month_keys, columns=warehouses, dtype=float).fillna(0)
        utilization = pd.DataFrame(index=month_keys, columns=warehouses, dtype=float).fillna(0)

        for wh in warehouses:
            cumulative[wh] = (in_pvt[wh] - out_pvt[wh]).cumsum()
            capacity = self._get_warehouse_capacity(wh)
            if capacity > 0:
                utilization[wh] = (cumulative[wh] / capacity * 100).round(2)

        # 6) 출력 테이블 구성 (4개 값 * 창고 수 + 누계)
        columns = ["입고월"]
        for wh in warehouses:
            columns.extend([f"입고_{wh}", f"출고_{wh}", f"누적_{wh}", f"이용률_{wh}_%"])
        columns.extend(["누계_입고", "누계_출고"])
            ...
            for wh in warehouses:
                row.extend(
                    [
                        int(in_pvt.at[m, wh]),
                        int(out_pvt.at[m, wh]),
                        int(cumulative.at[m, wh]),
                        utilization.at[m, wh],
                    ]
                )
            row.extend([int(in_pvt.loc[m].sum()), int(out_pvt.loc[m].sum())])
            rows.append(row)
        ...
        for col in df_out.columns[1:]:
            if "이용률" in col:
                total_row.append(round(df_out[col].mean(), 2))
            else:
                total_row.append(int(df_out[col].sum()))
        df_out.loc[len(df_out)] = total_row

        # 스냅샷 앵커링 적용
        try:
            snapshot = _build_latest_snapshot_from_master(stats)
            if snapshot:
                df_out = _anchor_cumulative_to_snapshot(df_out, snapshot)
                logger.info(f"스냅샷 앵커링 완료: {len(snapshot)}개 창고 보정")
        except Exception as e:
            logger.warning(f"스냅샷 앵커링 실패 (기존 로직 유지): {e}")
            # 실패해도 기존 df_out 반환하여 파이프라인 중단 방지

        logger.info(f"창고_월별_입출고 시트 생성 완료 (정규화 버전): {df_out.shape}")
        return df_out
    ...
    def create_multi_level_headers(self, df: pd.DataFrame, sheet_type: str) -> pd.DataFrame:
        ...
        if sheet_type == "warehouse":
            # 창고_월별_입출고용 Multi-Level Header (4개 데이터 타입)
            level_0 = ["입고월"]  # 첫 번째 컬럼
            level_1 = [""]
            ...
            for warehouse in warehouses:
                # 각 창고당 4개 컬럼
                level_0.extend([warehouse, warehouse, warehouse, warehouse])
                level_1.extend(["입고", "출고", "누적", "이용률_%"])
            ...
            multi_columns = pd.MultiIndex.from_arrays(
                [level_0, level_1], names=["Location", "Type"]
            )
    ...
    def generate_final_excel_report(self):
        ...
        # 시트 1: 창고_월별_입출고 (누적 재고 + 이용률 포함)
        warehouse_monthly = self.create_warehouse_monthly_sheet_enhanced(stats)
        ...
        # ✅ Source_Vendor 컬럼 사용 (Stage 1-2와 일관성) + SIEMENS 철자 수정
