...
# ------------------------------------------------------------------------------
# Date Range Calculation (for unified reporting across all sheets)
def calculate_max_date_from_dataframe(df, date_columns: list) -> str:
    """
    Calculate maximum date from DataFrame across multiple date columns.
    
    Used to unify date ranges across all Stage 3 report sheets by determining
    the latest date in raw data instead of using system clock.
    
    Args:
        df: pandas DataFrame containing date columns
        date_columns: List of column names to check for dates
    
    Returns:
        str: Maximum date in 'YYYY-MM' format
        
    Examples:
        >>> import pandas as pd
        >>> from datetime import datetime
        >>> df = pd.DataFrame({
        ...     "ETD/ATD": ["2025-08-15", "2025-10-20"],
        ...     "ETA/ATA": ["2025-09-01", "2025-10-25"]
        ... })
        >>> calculate_max_date_from_dataframe(df, ["ETD/ATD", "ETA/ATA"])
        '2025-10'
    """
    import pandas as pd
    from datetime import datetime
    ...
    for col in date_columns:
        if col in df.columns:
            ...
            col_max = col_dates.max()
            if pd.notna(col_max):
                if max_date is None or col_max > max_date:
                    max_date = col_max
    
    return max_date.strftime('%Y-%m') if max_date else datetime.now().strftime('%Y-%m')