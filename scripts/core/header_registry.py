...
class HeaderRegistry:
    ...
    def _initialize_definitions(self):
        ...
        self.register(
            HeaderDefinition(
                ...
                aliases=[
                    ...
                    "Package No.",
                    "PACKAGE NO",
                    "PackageNo",      # No space version
                    "packageno",      # Lowercase no space
                    "Package_No",     # Underscore version
                ],
        ...
        self.register(
            HeaderDefinition(
                ...
                description="Equipment or Purchase Order number (HITACHI: EQ No, SIEMENS: PO.No)",
                required=False,
                data_type="str",
            )
        )
        ...
        warehouse_locations = [
            ...
            (
                ...
                [
                    ...
                    "Hauler In",
                    "HAULER Indoor",
                    "HAULER",           # HAULER = Hauler Indoor (동일)
                    "Hauler",
                    "HAULER Warehouse",
                    "Hauler Warehouse",
                    "운송사실내",
    ...
    def get_warehouse_columns(self, use_primary_alias: bool = True) -> List[str]:
        """
        Get all warehouse location column names in correct order.
        
        This method provides the canonical ordering of warehouse columns
        used throughout the HVDC pipeline.
        
        Args:
            use_primary_alias: If True, return first alias (primary name like "DHL WH").
                              If False, return semantic key (like "dhl_wh").
        
        Returns:
            Ordered list of warehouse column names
            
        Example:
            >>> registry = HeaderRegistry()
            >>> registry.get_warehouse_columns()
            ['DHL WH', 'DSV Indoor', 'DSV Al Markaz', ...]
            >>> registry.get_warehouse_columns(use_primary_alias=False)
        ...
        warehouse_keys_in_order = [
            ...
            "dsv_mzp",
            "mosb",
            "hauler_indoor",  # "HAULER" and "Hauler Indoor" are the same (HAULER is an alias)
            "jdn_mzd",
        ]
        
        if use_primary_alias:
            return [self.get_definition(k).aliases[0] for k in warehouse_keys_in_order]
        else:
            return warehouse_keys_in_order

    def get_site_columns(self, use_primary_alias: bool = True) -> List[str]:
        """
        Get all site location column names in correct order.
        
        This method provides the canonical ordering of site columns
        used throughout the HVDC pipeline.
        
        Args:
            use_primary_alias: If True, return first alias (primary name like "MIR").
                              If False, return semantic key (like "mir").
        
        Returns:
            Ordered list of site column names
            
        Example:
            >>> registry = HeaderRegistry()
            >>> registry.get_site_columns()
            ['MIR', 'SHU', 'AGI', 'DAS']
            >>> registry.get_site_columns(use_primary_alias=False)
            ['mir', 'shu', 'agi', 'das']
        """
        site_keys_in_order = ["mir", "shu", "das", "agi"]
        
        if use_primary_alias:
            return [self.get_definition(k).aliases[0] for k in site_keys_in_order]
        else:
            return site_keys_in_order

    def get_date_columns(self, use_primary_alias: bool = True) -> List[str]:
        """
        Get all date/temporal column names including location columns (which are dates).
        
        This includes both explicit temporal columns (ETD/ATD, ETA/ATA) and
        location columns which store dates (warehouse dates, site dates).
        
        Args:
            use_primary_alias: If True, return first alias for each column.
                              If False, return semantic keys.
        
        Returns:
            List of date column names (not in a specific order)
            
        Example:
            >>> registry = HeaderRegistry()
            >>> date_cols = registry.get_date_columns()
            >>> 'ETD/ATD' in date_cols and 'DHL WH' in date_cols
            True
        """
        # Get all temporal headers
        temporal_headers = self.get_by_category(HeaderCategory.TEMPORAL)
        # Get all location headers (warehouses and sites are date columns)
        location_headers = self.get_by_category(HeaderCategory.LOCATION)
        
        all_date_headers = temporal_headers + location_headers
        
        if use_primary_alias:
            return [h.aliases[0] for h in all_date_headers if h.aliases]
        else:
            return [h.semantic_key for h in all_date_headers]
