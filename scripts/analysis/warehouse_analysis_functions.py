...
def calculate_warehouse_transition_plan(
    ...
    transition_logic = {
        "M44-INDOOR": lambda m: (
            3000 if m <= pd.to_datetime("2025-04-30") else
            2500 if m <= pd.to_datetime("2025-08-31") else
            2000 if m <= pd.to_datetime("2025-11-30") else
            1500
        ),
        "MARKAZ-INDOOR": lambda m: (
            0 if m <= pd.to_datetime("2025-04-30") else
            3000 if m <= pd.to_datetime("2025-05-31") else
            2000 if m <= pd.to_datetime("2025-10-31") else
            1500
        ),
        "MW4-INDOOR": lambda m: (
            0 if m <= pd.to_datetime("2025-05-31") else
            5000 if m <= pd.to_datetime("2025-06-30") else
            4500 if m <= pd.to_datetime("2025-07-31") else
            4000 if m <= pd.to_datetime("2025-08-31") else
            3500
        )
    }
...
def analyze_cost_efficiency(
    ...
    stats = {
        ...
        "cost_per_sqm": df[cost_col].sum() / df[sqm_col].sum() if df[sqm_col].sum() > 0 else rate_per_sqm,
        "efficiency_score": min(100.0, 100.0 * (1.0 - (df[cost_col].mean() - df[cost_col].min()) / df[cost_col].mean())) if df[cost_col].mean() > 0 else 100.0
    }
...
def compare_indoor_outdoor(
    ...
    outdoor_stats = {
        ...
        "trend": "increasing" if df[outdoor_col].iloc[-1] > df[outdoor_col].iloc[0] else "decreasing"
    }
    
    comparison = {
        "indoor_avg": indoor_stats["avg"],
        "outdoor_avg": outdoor_stats["avg"],
        "avg_difference_pct": ((outdoor_stats["avg"] - indoor_stats["avg"]) / indoor_stats["avg"] * 100) if indoor_stats["avg"] > 0 else 0,
        "indoor_trend": indoor_stats["trend"],
        "outdoor_trend": outdoor_stats["trend"],
        "utilization_ratio": outdoor_stats["avg"] / indoor_stats["avg"] if indoor_stats["avg"] > 0 else 0
    }
